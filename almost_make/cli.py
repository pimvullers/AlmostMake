...
JUST_FLAGS = {
    'help', 'keep-going', 'print-expanded', 'just-print', 'silent',
    'built-in-shell', 'print-directory', 'undefined-is-error'
}
...
def main(args=sys.argv):
    ...
    # Fill args from MAKEFLAGS (see
    # https://www.gnu.org/software/make/manual/make.html#How-the-MAKE-Variable-Works)
    # Previously-defined args take precedence.
    args = fillArgsFromEnv(args, "MAKEFLAGS", ARGUMENT_MAPPINGS, JUST_FLAGS)
    # For recursive calls to make.
    saveArgsInEnv(args, "MAKEFLAGS", NO_SAVE_ARGS)
    ...
    if 'help' in args:
        ...
        # Fills with macros from environment, etc.
        defaultMacros = macroUtil.getDefaultMacros()
        overrideMacros = {}
        ...
        if 'directory' in args:
            try:
                ...
                print(f"Error changing directories: {ex}")
                sys.exit(1)

        # If we know the path to the python interpreter...
        if sys.executable:
            defaultMacros["MAKE"] = sys.executable + " " \
        ...
        if 'jobs' in args:
            ...
            try:
                ...
                makeUtil.errorUtil.reportError(
                    "Invalid argument to --jobs. This must be an integer.")
        ...
        if 'expand-undefined-to' in args:
            makeUtil.setDefaultMacroExpansion(
                runner.stripQuotes(args["expand-undefined-to"]))
        ...
        if not os.path.exists(fileName):
            cprint(f"The file with name \"{fileName}\" was not found!\n",
                   FORMAT_COLORS['RED'])
        ...
        if 'print-expanded' not in args:
            # Run for each target.
            for target in targets:
                makeUtil.runMakefile(fileContents, target, defaultMacros,
                                     overrideMacros, file=fileName)
        else:
